...
function Home() {
    ...
    return (
        <Fragment>
            <div className={"row"}>
                <h1>총 <span style={{"fontSize":"30px", "color":"green"}}>{count}</span>개의 맛있는 레피시가 있습니다.</h1>
                <hr/>
            </div>
            <div className={"row"}>
                {html}
            ...
            <div className={"row"}>
                <div className={"text-center"}>
                    <input type={"button"} value={"이전"} className={"btn btn-sm btn-danger"} onClick={prevHandler}/>
                    {curpage} page / {totalpage} pages
                    <input type={"button"} value={"다음"} className={"btn btn-sm btn-primary"} onClick={nextHandler}/>
                </div>
            </div>
        </Fragment>
    )
